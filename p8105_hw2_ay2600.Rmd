...
We clean the `mr_trash_wheel` data, provides detail records of waste collection, capturing various information of waste items collected, ranging from years `r mr_trash_wheel |> pull(year) |> min()` to `r mr_trash_wheel |> pull(year) |> max()`. Each entry in the dataset represents a unique colelction event, with data on the date of collection, for example, like weight of waste `weight_tons`, volume of waste `volume_cubic_yards` etc. During the cleaning process, the `janitor` was used. Also, removed rows with missing dumpster values. Unnecessary columns `x15` and `x16` are dropped. We then calculate homes_powered using the formula `(weight_tons * 500) / 30` and add a `wheel_name` column with the value "Mr. Trash Wheel" for identification purposes. Additionally, the `year` column is converted to numeric values to facilitate subsequent analyses.
...
- In the `mci_baseline_full`, there are `r nrow(mci_baseline_full)` observations and `r ncol(mci_baseline_full)` variables. This data provided information of participants in the study. 

- During the cleaning process, `janitor::clean_names()` function was used to standardize the variable names. `mutate` function recode the `sex` variable into `Male` and `Female` categories based on the original numeric coding of 1 and other values. Similarly, the `apoe4` variable is recoded to `Carrier` and `Non carrier` categories. The `age_at_onset` is converted into numeric. Lastly, assigned a new dataset `mci_baseline`, which participants who do not have MCI excluded.
